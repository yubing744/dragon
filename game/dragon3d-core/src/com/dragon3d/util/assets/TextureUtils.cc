...
#include <com/dragon3d/util/assets/TextureUtils.h>
...
void Texture::load(const char* strFileName) {
    if(!strFileName)  
        return;
...
void Texture::loadBMP(const char *strFileName){
    /*
...
void Texture::loadJPG(const char *strFileName){
    /*
...
void Texture::loadTGA(const char *strFileName){
    ...
    res->read(&length, sizeof(ubyte), 1);
    res->seek(1, SEEK_CUR); 
    res->read(&imageType, sizeof(ubyte), 1);
    
    res->seek(9, SEEK_CUR); 

    res->read(&width,  sizeof(ushort), 1);
    res->read(&height, sizeof(ushort), 1);
    res->read(&bits,   sizeof(ubyte), 1);
    ...
    else
    {
        ...
        data = ((unsigned char*)malloc(sizeof(unsigned char)*stride*height));
        ubyte *pColors = ((ubyte*)malloc(sizeof(ubyte)*channels));
        
        while(i < width*height)
        {
            res->read(&rleID, sizeof(ubyte), 1);
            ...
            if(rleID < 128)
            {
                ...
                while(rleID)
                {
                    res->read(pColors, sizeof(ubyte) * channels, 1);
