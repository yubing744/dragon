...
static bool load_luts(void)
{
   ...
   for (i = 0; i < glsl_shader->luts; i++)
   {
      ...
      glTexImage2D(GL_TEXTURE_2D,
            0, glsl_shader->gfx_use_rgba ? GL_RGBA : RARCH_GL_INTERNAL_FORMAT32,
            img.width, img.height, 0,
            glsl_shader->gfx_use_rgba ? GL_RGBA : RARCH_GL_TEXTURE_TYPE32,
      ...
      const struct gl_tex_info *fbo_info, unsigned fbo_info_cnt)
{
   ...
   if (uni->frame_direction >= 0)
      glUniform1i(uni->frame_direction, glsl_shader->frame_is_reverse ? -1 : 1);
